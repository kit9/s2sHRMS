...
if (isset($_POST["SearchMissingIntime"])) {
    ...
    if ($company_id <= 0) {
        ...
        $list = array();
        //Headers Array
        $columns = array('Employee Code', 'Full Name', 'Designation', 'Department', 'Subsection', 'in_time');
	...
	dep.department_title,
        ...
        WHERE
                in_time = '$zero' OR ISNULL(in_time)
        AND date = '$temp_date'
        AND tmp.emp_code IN(
                SELECT
                        emp_code
        ...
        $cWorkSheet->setCellValueByColumnAndRow(0, 2, "Missing In-Time Report");
        ...
        $cWorkSheet->setCellValueByColumnAndRow(5, 8, "In Time");
        ...
        for ($row = 9; $row < count($objects) + 9; $row++) {
            ...
            $cWorkSheet->setCellValueByColumnAndRow(4, $row, $objects["$xx"]["emp_subsection"]);
            $cWorkSheet->setCellValueByColumnAndRow(5, $row, $objects["$xx"]["in_time"]);
            $xx++;
        }

        //Define filename and create excel report
        $objWriter = new PHPExcel_Writer_Excel2007($createPHPExcel);
        $filename = $company_id . rand(0, 9999999) . "_Missing_Intime.xlsx";
...
<div class="widget" style="background-color: white;">
    <div class="widget-head"><h6 class="heading" style="color:whitesmoke;">Create Missing In-Time Report</h6></div>
