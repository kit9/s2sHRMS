            ...
            foreach ($object_2 as $otKey => $val2) {
                ...
                if ($replacement_status != '') {
                    if ($replacement_status == "P") {
                        $real_ot = date("H:i:s", strtotime($val2["ot_hours"]));
                        $std = date("H:i:s", strtotime("2:00:00"));
                        if ($real_ot > $std) {
                            //calculate extra OT
                            $extra_ot = date("H:i", strtotime($real_ot) - strtotime($std));
                            //push extra ot
                            array_push($tmpArray, $extra_ot);
                            //array_push($tmpArray,"88:00:00");
                            /*
                             * Collect variables for total OT
                             * Other calculation will be out of this loop.
                             * Other caculation will be each employee
                             */
                            $x_time = $extra_ot;
...
//                            $OT = strtotime(implode(":", $time_array));
                            ...
                            //Make final OT
                            $newot=date("h:i",  strtotime($extra->OtDuductionBYHour($OT)));
                            array_push($tmpArray,$newot);
                            $x_time_array = explode(":", $newot);
                            $xhours += $x_time_array[0];
                            $xMinutes += $x_time_array[1];
                            
                            //# code is commented due to additional OT
//                            $tem_time_diff = date("H:i", $OT);
//                            $mod_time_array = explode(":", $tem_time_diff);
//                            $mod_hours = $mod_time_array[0];
//
//                            if ($tem_time_diff == "00:00:00" || $tem_time_diff == 0) {
//                                array_push($tmpArray, " ");
//                            } else {
//                                if ($mod_hours >= 7) {
//                                    $tem_time_diff_2 = date("H:i:s", strtotime("$tem_time_diff -1 hour"));
//                                }
//                                else
//                                {
//                                    $tem_time_diff_2 = date("H:i:s", strtotime($tem_time_diff));
//                                }
//                                $std_tem_time_diff = date("H:i:s", strtotime("08:00:00"));
//                                if ($tem_time_diff_2 > $std_tem_time_diff) {
//                                    $additinal_ot = date("H:i", (strtotime($tem_time_diff_2) - strtotime($std_tem_time_diff)));
//                                    array_push($tmpArray, $additinal_ot);
//                                    //Prepare data for total additinal OT
//                                    $x_time = $additinal_ot;
//                                    $x_time_array = explode(":", $x_time);
//                                    $xhours += $x_time_array[0];
//                                    $xMinutes += $x_time_array[1];
...
//                                }
//                            }
//                            #code is commented due to weekend additional OT
//                        } else {
//                            array_push($tmpArray, " ");
//                        }
                    }
                    ...
                    //Calculate OT for day type W,H or F
...
//                            $OT = strtotime(implode(":", $time_array));
                            ...
                            $tem_time_diff = date("H:i", $OT);
                            $mod_time_array = explode(":", $tem_time_diff);
                            $mod_hours = intval($mod_time_array[0]);

                            // test   $tem_time_diff="11:11:11";
...
//                        }
            ...
            //Creating final data array for each employee
        ...
        $filename = $company_id . rand(0, 9999999) . "AdditionalOtReprot.xlsx";
...
<div class="widget" style="background-color: white;">
    <div class="widget-head"><h6 class="heading" style="color:whitesmoke;">Create Additional OT Report</h6></div>
