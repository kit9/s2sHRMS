...
/*
 * Author: Rajan Hossain, Tariqule, Jobayer Rabbi,Md Mahamudur Zaman Bhuyan
...
foreach ($attns as $attn) {
    ...
    if ($delete_edited_data == 0 && $manual_edit_flag == 1) {
        ...
        // echo "<br />";
        // echo "Shift Start Time: $shift_start_time <br />";
        // echo "Shift Start Time: $shift_end_time <br />";
        // echo $target_working_hours_frmt; 
        // exit();
        ...
        $entry_buffer_minute = '';
        $exclude_early_entry_minutes = 0; //False;
        if (count($late_buffer_minute) > 0) {
            $entry_buffer_minute = $late_buffer_minute{0}->late_buffer_minute;
            $exclude_early_entry_minutes = $late_buffer_minute{0}->exclude_early_entry_minutes;
        }
...
//        exit();
        if ($sat_end_day > 1) {
            ...
            if (count($job_cards_night_shift_ot) > 0) {
                ...
                /**
                 * check if early minutes is set to be excluded
                 * if so, then check if intime is before than shift start time
                 * if so, then replace in time with shift start time 
                 * rename intime as a different intime, so that same variables
                 * do not get confused and actual intime as entry time 
                 * is not replaced in any way.
                 */
                $finalized_in_time = '';
                if ($exclude_early_entry_minutes == 1) {
                    if ($in_time < $shift_start_time) {
                        //Replace in time with shift start time
                        $finalized_in_time = $shift_start_time;
                    } else {
                        $finalized_in_time = $in_time;
                    }
                } else {
                    //If exclude early minute is turned off
                    $finalized_in_time = $in_time;
                }

                if ($is_ot_eligible == 1) {
                    if ($in_time != $out_time) {
                        $office_end_time = date("H:i:s", strtotime("+$tw_hours hours $tw_minutes minutes $tw_seconds seconds", strtotime($finalized_in_time)));
