...
//                        foreach ($emp_year_pf as $key=>$val) {
////                            print_r($val);
//                           $new_array["$key"]["year"] = $val->PFDY_year;
//                                foreach ($pfd_detail as $rb) {
//                                    if ($rb->pfd_year == $val->PFDY_year) {
//                                        foreach ($month as $keymon => $valmon) {
//                                            if ($keymon == $rb->pfd_month) {
//                                                $new_array["$key"]["ee"] =$rb->pfd_emp_amount;
//                                                $new_array["$key"]["ec"] =$rb->pfd_com_amount;
//                                            }
//                                        }
//                                    }
//                                }
//                            $new_array["$key"]["other"] = $val->PFDY_pfd_others; 
//                            $new_array["$key"]["total"] = $val->PFDY_pfd_total;
//                        }
//                    }
//                $con->debug($new_array);
//                exit();
//======================================================================================================
    ...
    array_push($header_array, "Year", "January", " ", "February", " ", "March", " ", "April", " ", "May", " ", "June", " ", "July", " ", "August", " ", "September", " ", "October", " ", "November", " ", "December", " ", "Others", "Provident Fund Total");
...
//        }
//
//        if (count($Prof_detail) > 0) {
//            foreach ($Prof_detail as $key => $val) { 
//                $i = 0;
//                $Arr["$key"][$i] = $val->PFDY_year;
//                $i++;
//
//                foreach ($pfd_detail as $al) {
//                    if ($val->PFDY_year == $al->pfd_year) {
...
//                }
//                
//                $Arr["$key"][$i] = $val->PFDY_pfd_others;
//                $i++;
//                $Arr["$key"][$i] = $val->PFDY_pfd_total;
//                $i++;
//            }
//        }
    /** collect and summ up leave data */
//        $dates = $con->SelectAllByCondition("date", "date between '$build_first_date' AND '$formatted_today' and company_id='$companies'");
...
//           echo  $i;
//          $j = 0;
//       foreach ($pfd_detail as $pd){
//                if($pd->pfd_year == $Prof_detail[$i]->PFDY_year){
//                echo $j++;
//                   $con->debug($pd->pfd_year);
//                   $con->debug($Prof_detail[$i]->PFDY_year);
////               $con->debug($pd->pfd_emp_amount);
////               $con->debug($pd->pfd_com_amount);
//                }
//            }
//        }
    //=================
...
//        for ($j = 0; $j <= $countCol - 1; $j++) {
        ...
        $cWorkSheet->getStyle("A5:AA5")->applyFromArray($styleArray1);
...
//                $pfd_year_detail = $con->QueryResult($pf_yr_query);
                foreach ($month as $key => $val) {
                    if ($key == $pd->pfd_month) {
                        $cWorkSheet->setCellValueByColumnAndRow($j, $i + 7, $pd->pfd_emp_amount);
                        $j++;
                        $cWorkSheet->setCellValueByColumnAndRow($j, $i + 7, $pd->pfd_com_amount);
                        $j++;
//              for ($a = 1; $a < 12; $a++) {
//                 $cWorkSheet->setCellValueByColumnAndRow($a, $i + 5, $Arr["$rowCount"]["$j"]);
                    }else{
        ...
        $pfd_total = $pfd_total + $Prof_detail["$i"]->PFDY_pfd_total;
//         $j++;
//      } 
//      $rowCount++;
    }
...
<div class="widget" style="background-color: white;">
    ...
    <!--                    <th data-field="ee" style="border-bottom: 1px solid gray;word-wrap: break-word; white-space: pre-wrap;text-align:center;">EE</th>
                        ...
                        <?php } ?>
                        <th data-field="pfd_others" style="border-bottom: 1px solid gray;width: 120px;"></th>
                        <th data-field="pfd_total" style="border-bottom: 1px solid gray;width: 160px;"></th>
                    </tr>
                </thead>
                <tbody>
                <?php
...
//                                }
                            ...
                            <?php
                            $grand_total = $grand_total + $ey->PFDY_pfd_total;
                        }
                    }
                    ?> 
                </tbody>
