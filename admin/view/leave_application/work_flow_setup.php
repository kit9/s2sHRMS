...
//    exit();
    ...
    $countempauth = count($_POST['employee']);
    if ($countempauth != 0) {
        $a=0;
        $b=0;
        $c=0;
        $d=0;
        foreach ($_POST['employee'] as $emat):
            //start full process here
            $sqlSearch = "SELECT * FROM approval_workflow_settings WHERE emp_code_auth='" . $emat . "' AND department_id='" . $department . "'";
...
//            }
            ...
            //end full process here
        endforeach;
        
        if($a!=0)
        {
            $msg= "Workflow settings information is successfully saved for (".$a.") employee.";
            $msg .= " Already setup (".$b."), and failed to setup (".$c.") employee.";
        }
        else
        {
            $msg = "Setup already setup for (".$b."), and failed to setup for (".$c.") employee.";
        }
        
        
    } else {
        $err = "Please Select a Employee for Workflow Setup.";
    }
}
?>
...
<style type="text/css">
    span.k-i-calendar,
    span.k-icon{ margin-top:3px !important; }
</style>  
...
?> class="test" value="individual" id="workflow_approval_type_ind" name="workflow_approval_type" >Individual Approval &nbsp; &nbsp;  
            ...
            if ($ends_at_first_rejection == 1) {
                echo 'checked="true"';
            }
            ?>  id="ends_at_first_rejection" name="ends_at_first_rejection" ><span id="rej_label"> Ends at First Rejection &nbsp; &nbsp;</span> 
        </span>
    </div>
        ...
        $(document).ready(function () {
            $("#workflow_approval_type_ind").click(function () {
                ...
                $.ajax({
                    ...
                    data: {workflow_approval_type: workflow_approval_type},
                    dataType: "json",
...
<?php include("../../layout/msg.php"); ?>
        ...
        <form method="post">
            ...
            <div class="col-md-4">
                ...
                <script>
                    $(document).ready(function () {
                        $("#departments").change(function () {
                            if ($(this).val() != '') {
                                $.get("../../controller/employee_filter.php", {'dept': $(this).val()}, function (returnData) {
                                    var obj = returnData;
                                    var htmldata = '';
                                    $.each(obj['data'], function (key, index) {
                                        console.log(index.emp_code);
                                        htmldata += '<option value="' + index.emp_code + '">' + index.emp_firstname + '</option>';
                                    });

                                    $("#employee").html(htmldata);
                                    //console.log(obj);
                                });
                            }
                        });
                ...
                <select multiple id="employee" name="employee[]" style="width: 80%; height: 200px;">
...
<script id="edit_workflow" type="text/x-kendo-template">
    <a href="edit_work_flow_setup.php?leave_workflow_id=#= leave_workflow_id #" class="k-button">Edit</a>
</script>
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        var dataSource = new kendo.data.DataSource({
            ...
            schema: {
                ...
                model: {
                    id: "department_id",
                    fields: {
                        auth_emp: {editable: false, nullable: true},
                        department_id: {editable: false, nullable: true},
                        leave_workflow_id: {type: "number"},
                        emp_code_auth: {type: "string"},
                        ...
                        emp_email_office: {type: "string", validation: {required: "Invalid "}},
                        //status: {type: "boolean"}
                    }
                }
            }
        ...
        jQuery("#grid").kendoGrid({
            ...
            columns: [
                {field: "auth_emp", title: "Employee Authorization", id: "auth_emp", width: "190px"
                ...
                {field: "emp_email_office", title: "Mail", id: "emp_email", width: "150px"},
...
//                transport: {
//                    read: "../../controller/employee_filter.php"
//                },
//                schema: {
//                    data: "data"
//                }
...
//        var employee = $("#employee").kendoMultiSelect({
//            cascadeFrom: "departments",
//            optionLabel: "Select Employees...",
//            dataTextField: "emp_firstname",
//            dataValueField: "emp_code",
...
//                transport: {
//                    read: {
//                        url: "../../controller/employee_filter.php",
//                    }
//                },
//                schema: {
//                    data: "data"
