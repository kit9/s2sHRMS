...
$delete_flag=false;
$mpquery = "SELECT * FROM module_permission WHERE emp_code='$emp_code_nav' AND module_page_title='Leave Application' AND perdelete='yes'";
$sqlchkper=$con->FlyQuery($mpquery,1);
if($sqlchkper>0)
{
    $delete_flag=true;
}
...
<div class="col-md-6" style="padding-left: 0px;">
    <span>Leave Applications Pending for Your Approval</span>
</div>
<hr />
<div class="clearfix"></div>

<!--Remarks for both approval or reject-->
<div class='col-md-6' style='padding-left:0px;'>
	<span><b>Add a remark for your review: (optional)</b></span>
	<br />
	<span style="font-size:13px; color:darkgrey;">
		#Note: Please do not make this note too large.
		You can use this remark for both approval and reject. 
	</span>
	<br />
	<br />
	<input type='text' id='review_remark' class='k-textbox' style="width:100%;">
	<div class='clearfix'></div>
	<br />
</div>
<div class='clearfix'></div>
...
<script id="details-two-template" type="text/x-kendo-template">
    ...
    # }# 
        <?php if($delete_flag==true && $emp_code_nav=='3016'){ ?>
      #if(aws_status == "pending" || aws_status == "rejected") { #    
            <a style="font-size:11px; text-decoration: none;" onclick="javascript:leave_delete(#= leave_application_master_id #);" class="k-button k-grid-even"><span class="k-icon k-delete"></span> Delete</a>
      #}#
      <?php } ?>
</script>
...
<script>
    function leave_delete(master) {
	//Collect remark value from remark input
        $.ajax({
            url: "../../controller/leave_management_controllers/leave_applications_delete.php",
            data: {master: master},
            type: "POST",
            dataType: "json",
		    ...
		    //Empty remark field once application is successfully submitted
                    $("#review_remark").val();
	...
	//Collect remark from review field
	var review_remark = $("#review_remark").val();
