...
/*
 * Parent class holding all child classes
 */
...
class Config {
    ...
    public function open() {
        $con = mysqli_connect("localhost", "root", "@Admin#2016", "s2s_payroll");
        return $con;
    }
...
//        if ($object == "leave_application_details") {
//            echo $query = "INSERT INTO `$object` SET $fields";
//            exit();
//        } else {
    ...
    function adminlogin($username, $password) {
        ...
        $fields = " emp_code='" . mysqli_real_escape_string($con, $username) . "' and password='" . mysqli_real_escape_string($con, $password) . "'";

        $query = "SELECT * FROM `tmp_employee` WHERE $fields";
        $result = mysqli_query($con, $query);
        if ($result) {
            $count = mysqli_num_rows($result);
            $this->close($con);
    ...
    function FlyPrepareMulti($query) {
        $count = 0;
        $fields = '';
        $con = $this->open();
//        echo $query;
//        exit();
    ...
    function baseUrl($suffix = '') {
        ...
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "s2s_payroll/admin/";
        $suffix = ltrim($suffix, '/');
    ...
    function SiteBaseUrl($suffix = '') {
        $protocol = strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "s2s_payroll/";
        $suffix = ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
