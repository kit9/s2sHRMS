...
if ($verb == "GET") {
    $arr = array();
    $rs = mysqli_query($con->open(), "SELECT u.u_id, u.u_name, u.u_website, u.u_address, u.u_phone, u.country_id, c.country_name,u.city_id,u.u_image, cn.city_name FROM tbl_university u,tbl_country c, tbl_city cn WHERE u.country_id=c.country_id AND u.city_id=cn.city_id ");
...
if ($verb == "POST") {
   ...
   $rs = mysqli_query($con->open(),"UPDATE tbl_university SET u_name = '" .$u_name ."', country_id = '" .$country_id ."', city_id = '" .$city_id ."', u_address = '" .$u_address ."', u_website = '" .$u_website ."', u_image = '" .$u_image ."', u_phone = '" .$u_phone ."' WHERE u_id = " .$u_id);
...
$sql = "insert into tbl_university(u_name,u_address,u_website,u_image, u_phone,country_id,city_id) values('$u_name','$u_address','$u_website', '$u_image','$u_phone','$country_id','$city_id')";
