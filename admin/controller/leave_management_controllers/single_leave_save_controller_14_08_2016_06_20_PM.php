    ...
    if ($availability_flag == 0) {
        ...
        foreach ($dates as $date) {
            ...
            if ($last_id != 0) {
                ...
                $details_array = array("leave_application_master_id" => $last_id,
                    ...
                    "is_half" => $is_half,
                    "day_part" => $day_part);
                if (!empty($replacement_date)) {
                    $details_array2 = array("emp_code" => $emp_code,
                        "replacement_date" => $replacement_date);
                } else {
                    $details_array2 = array("emp_code" => $emp_code);
                }
                $detail_merge_array = array_merge($details_array, $details_array2);

                $con->insert("leave_application_details", $detail_merge_array);

//                if ($is_wh_included == "false" && ($day_type_id != 2 && $day_type_id != 3 && $day_type_id != 4)) {
            ...
            if (!empty($HOD_supervisor_id)) {
                ...
                if ($con->insert("approval_workflow_status", $status_insert_array_HOD) == 1) {
                    ...
                    if (isset($hod_supervisor{0}->emp_email_office)) {
                        ...
                        //Find leave details
                        $leave_master = $con->FlyQuery("SELECT a.*,t.emp_firstname,t.emp_lastname,lad.leave_type_id,lp.leave_title FROM `leave_application_master` as a
                                                        LEFT JOIN tmp_employee as t on t.emp_code=a.emp_code 
                                                        LEFT JOIN leave_application_details as lad on lad.`leave_application_master_id`=a.`leave_application_master_id` 
                                                        LEFT JOIN leave_policy as lp on lp.leave_policy_id=lad.leave_type_id  WHERE a.leave_application_master_id='$last_id'");
                        $ap_start_date = $leave_master{0}->start_date;
                        $ap_end_date = $leave_master{0}->end_date;
                        $applied_user_code = $leave_master{0}->emp_code;
                        $applied_user_full_name = $leave_master{0}->emp_firstname." ".$leave_master{0}->emp_lastname;
                        $applied_leave_reason = $leave_master{0}->reasons;
                        $applied_leave_type = $leave_master{0}->leave_title;
                        ...
                        if ($ap_start_date == $ap_end_date) {
                            ...
                            $message .=$applied_user_full_name." (Emp. ID- ".$applied_user_code.") has submitted a leave application for  " . $ap_start_date . " which is waiting for your review.
                                        <br /><br />
                                        Leave Type: ".$applied_leave_type."
                                        <br /><br />
                                        Leave Reason: ".$applied_leave_reason;
                            
                            $message .="<br /><br />URL: http://59.152.97.62:5000/s2s_payroll/admin/login.php";
                            ...
                            //$message .= " has submitted a leave application for " . $ap_start_date . " ";
                            //$message .= "which is waiting for your review.";
                            $message .=$applied_user_full_name." (Emp. ID- ".$applied_user_code.") has submitted a leave application from " . $ap_start_date . " to ".$ap_end_date." which is waiting for your review.
                                        <br /><br />
                                        Leave Type: ".$applied_leave_type."
                                        <br /><br />
                                        Leave Reason: ".$applied_leave_reason;
                 ...
                 */
                $master_delete_array = array("leave_application_master_id" => $last_id);
                $con->delete("leave_application_master", $master_delete_array);
                $details_delete_array = array("leave_application_master_id" => $last_id);
                $con->delete("leave_application_details", $details_delete_array);
                $return_array = array("error_msg" => "Leave request failed. Probably a supervisor is not yet assigned to your department. Please contact your supervisor for further assistance.");
            }
        } else {
            $mail_flag = 1;
            $workflow_steps = $con->FlyQuery("SELECT * FROM approval_workflow_settings WHERE department_id='" . $department_id . "' AND emp_code_auth='" . $emp_code . "'");
            ...
            if (count($workflow_steps) > 0) {
                foreach ($workflow_steps as $steps) {
                    ...
                    if ($con->insert("approval_workflow_status", $status_insert_array) == 1) {
                        ...
                        $details_delete_array = array("leave_application_master_id" => $last_id);
                        $con->delete("leave_application_details", $details_delete_array);
                        $return_array = array("error_msg" => "Leave request failed. Something went wrong!");
                    }
                }
            } else {
                /*
