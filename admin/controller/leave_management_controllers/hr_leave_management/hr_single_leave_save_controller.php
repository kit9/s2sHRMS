...
//Mail configuration 
include '../../../config/class2.mail.php';
//Connection and operation class for low level interaction
$con = new Config();
//Instamtiate mail parent class
...
if ($start_date == '') {
    ...
    $return_array = array("error_msg" => "Number of total days were not calculated properly. Check your selected date range carefully.");
} else {
    ...
    if ($availability_flag == 0) {
        $return_array = array("error_msg" => "This employee is not authorized to avail this leave type untill he/she completes <b>" . $available_after_months . " months.</b>");
        ...
        if ($is_HOD == 'yes') {
            ...
            if (count($workflow_steps) > 0) {
                foreach ($workflow_steps as $steps) {
                    ...
                    $return_array = array(
                        "error_msg" => "A leave request is submitted. Once reviewed, applicant should recieve a confirmation email.",
                        "success_flag" => "yes"
                    );
                }
            } else {
