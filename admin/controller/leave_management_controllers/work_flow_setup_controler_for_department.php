...
if ($verb == "GET") {
    $arr = array();
    $query = "select approval_workflow_settings.*, department.department_title, tmp_employee.emp_firstname, emp_email_office FROM
...
if ($verb == "DELETE") {
    ...
    $open = $con->open();
    $query = "DELETE FROM approval_workflow_settings WHERE department_id='" . mysqli_real_escape_string($open, $department_id) . "'";
    $rs = mysqli_query($open, $query);
    if ($rs) {
        echo "" . $department_id . "";
        $con->close($open);
